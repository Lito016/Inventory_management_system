...
import { useFulfillments, useFulfillmentItems, useCreateFulfillment, useUpdateFulfillmentStatus } from '@/hooks/use-b2b';
import { useAuth } from '@/hooks/use-auth';
import { usePagination } from '@/hooks/use-pagination';
import { handleSupabaseError } from '@/lib/utils/api-errors';
import { formatDate, getTodayISO } from '@/lib/utils/dates';
import { FULFILLMENT_STATUS_COLORS } from '@/lib/constants';
import type { B2BFulfillment } from '@/types';
...
export function FulfillmentsPage() {
  ...
  const { data, isLoading, error } = useFulfillments({ status: statusFilter || undefined, page });
  const { data: detailItems } = useFulfillmentItems(detailId);
  ...
  const { register, handleSubmit, reset, control, formState: { isSubmitting } } = useForm({
    defaultValues: { customer_id: '', fulfillment_date: getTodayISO(), notes: '', items: [{ product_id: '', quantity: '1', unit_price: '' }] },
  });
  const { fields, append, remove } = useFieldArray({ control, name: 'items' });

  const columns: ColumnDef<B2BFulfillment & { customer: { name: string } }>[] = [
    { key: 'fulfillment_date', header: 'Date', render: (r) => formatDate(r.fulfillment_date) },
    ...
    { key: 'actions', header: 'Actions', render: (r) => (
      <div className="flex gap-1">
        ...
        {r.status === 'Pending' && <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); updateStatus.mutate({ id: r.id, status: 'Cancelled' }); }}>Cancel</Button>}
        <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); setDetailId(r.id); }}>Items</Button>
      </div>
    )},
  ];

  async function onSubmit(data: { customer_id: string; fulfillment_date: string; notes: string; items: { product_id: string; quantity: string; unit_price: string }[] }) {
    if (!user) return;
    setFormError('');
    try {
      await createFulfillment.mutateAsync({
        ...
        created_by: user.id,
        items: data.items.map(i => ({ product_id: i.product_id, quantity: i.quantity, unit_price: i.unit_price })),
      });
      setCreateOpen(false);
      reset();
  ...
  return (
    <>
      ...
      <PageContainer title="Fulfillments" actions={<Button onClick={() => setCreateOpen(true)}><Plus className="h-4 w-4 mr-1.5" />New Fulfillment</Button>}>
      <div className="mb-4 w-full sm:w-44">
        <Select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} options={[{ value: '', label: 'All Statuses' }, ...['Pending', 'In Progress', 'Completed', 'Cancelled'].map(s => ({ value: s, label: s }))]} />
      </div>
      ...
      <Table columns={columns} data={data?.items ?? []} loading={isLoading} emptyMessage="No fulfillments found." />

      {/* Create Modal */}
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="New Fulfillment" size="lg"
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Input label="Fulfillment Date" type="date" required {...register('fulfillment_date')} />
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <Button type="button" variant="secondary" size="sm" onClick={() => append({ product_id: '', quantity: '1', unit_price: '' })}>
      ...
      {/* Items Detail Modal */}
      <Modal open={!!detailId} onClose={() => setDetailId(null)} title="Fulfillment Items" size="md">
            ...
            <tbody>
              {detailItems.map(item => (
                <tr key={item.id} className="border-b border-gray-100">
                  ...
                  <td className="py-2 text-sm text-right font-medium">₱{(parseFloat(item.quantity) * parseFloat(item.unit_price)).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
