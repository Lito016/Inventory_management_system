...
import { usePreOrders, usePreOrderItems, useCreatePreOrder, useUpdatePreOrderStatus } from '@/hooks/use-b2b';
import { useAuth } from '@/hooks/use-auth';
import { usePagination } from '@/hooks/use-pagination';
import { handleSupabaseError } from '@/lib/utils/api-errors';
import { formatDate, getTodayISO } from '@/lib/utils/dates';
import { B2B_PRE_ORDER_STATUS_COLORS } from '@/lib/constants';
import type { B2BPreOrder } from '@/types';
...
export function PreOrdersPage() {
  ...
  const { data, isLoading, error } = usePreOrders({ status: statusFilter || undefined, page });
  const { data: detailItems } = usePreOrderItems(detailId);
  ...
  const { register, handleSubmit, reset, control, formState: { isSubmitting } } = useForm({
    defaultValues: { customer_id: '', order_date: getTodayISO(), notes: '', items: [{ product_id: '', quantity: '1', unit_price: '' }] },
  });
  const { fields, append, remove } = useFieldArray({ control, name: 'items' });

  const columns: ColumnDef<B2BPreOrder & { customer: { name: string } }>[] = [
    ...
    { key: 'actions', header: 'Actions', render: (r) => (
      <div className="flex gap-1">
        ...
        {(r.status === 'Draft' || r.status === 'Submitted') && <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); updateStatus.mutate({ id: r.id, status: 'Cancelled' }); }}>Cancel</Button>}
        <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); setDetailId(r.id); }}>Items</Button>
      </div>
    )},
  ];

  async function onSubmit(data: { customer_id: string; order_date: string; notes: string; items: { product_id: string; quantity: string; unit_price: string }[] }) {
    if (!user) return;
    setFormError('');
    try {
  ...
  return (
    <>
      ...
      <PageContainer title="Pre-Orders" actions={<Button onClick={() => setCreateOpen(true)}><Plus className="h-4 w-4 mr-1.5" />New Pre-Order</Button>}>
      <div className="mb-4 w-full sm:w-44">
        <Select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} options={[{ value: '', label: 'All Statuses' }, ...['Draft', 'Submitted', 'Converted', 'Cancelled'].map(s => ({ value: s, label: s }))]} />
      </div>
      ...
      <Table columns={columns} data={data?.items ?? []} loading={isLoading} emptyMessage="No pre-orders found." />

      {/* Create Modal */}
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="New Pre-Order" size="lg"
      ...
      {/* Items Detail Modal */}
      <Modal open={!!detailId} onClose={() => setDetailId(null)} title="Pre-Order Items" size="md">
